...
const useCommand = (question: QuestionItem) => {
  ...
  const getMultiLanguageAnswer = useCallback(
    (option: string) => {
      const answerOption = question.answerOptions?.find(
        (opt) => opt.text.en === option || opt.text.fi === option,
      );
  ...
  const handleMultiChoiceClick = useCallback(
    (option: string) => {
      ...
      try {
        const prevAnswerData = userInfo.questionAnswers[
          question.id
        ] as AnswerData;
        const prevSelected = {
          en: Array.isArray(prevAnswerData?.answer?.en)
            ? prevAnswerData.answer.en
            : [],
          fi: Array.isArray(prevAnswerData?.answer?.fi)
            ? prevAnswerData.answer.fi
            : [],
        };
        ...
        const totalScore = newSelected.en.reduce(
          (acc, opt) => acc + calculateScore(opt),
          0,
        );
